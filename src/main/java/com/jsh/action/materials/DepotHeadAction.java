        ...
        try {
            ...
            if (null != dataList) {
                for (DepotHead depotHead : dataList) {
                    ...
                    // 根据depotHead id查找对应的depotItem信息------start
                    PageUtil<DepotItem> depotItemPageUtil = new PageUtil<DepotItem>();
                    // 设置查询条件
                    Map<String, Object> condition = new HashMap<String, Object>();
                    condition.put("HeaderId_n_eq", depotHead.getId());
                    condition.put("Id_s_order", "asc");
                    depotItemPageUtil.setAdvSearch(condition);
                    depotItemService.find(depotItemPageUtil);

                    String materialInfo = getMaterialInfo(depotItemPageUtil.getPageList());
                    // 根据depotHead id查找对应的depotItem信息------stop

                    // 根据查找条件中的商品名称，在materialInfo中进行过滤----start
                    if (StringUtils.isNotBlank(model.getMaterialName())) {
                        if (!materialInfo.contains(model.getMaterialName())) {
                            continue;
                        }
                    }
                    // 根据查找条件中的商品名称，在materialInfo中进行过滤----stop
                    ...
                    item.put("MaterialInfo", materialInfo);
...
//        condition.put("Number_s_like", model.getNumber());
     ...
     * 获取商品信息
     *
     * @param depotItemList depotItemList
     * @return String
     */
    private String getMaterialInfo(List<DepotItem> depotItemList) {
        StringBuilder materialInfo = new StringBuilder();
        for (DepotItem item : depotItemList) {
            if (item.getMaterialId() != null) {
                materialInfo.append(item.getMaterialId().getName()).append(",");
            }
        }
        String info = materialInfo.toString();
        return StringUtils.isNotBlank(info) ? info.substring(0, info.length() - 1) : "";
    }
